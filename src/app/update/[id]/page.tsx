...
const ViewPage = ({params}: {params: Promise<{id: string}>}) => {
        ...
        const fetchIngredients = async () => {
            const fetchCocktailIngredients = async () => {
                try {
                    ...
                    return response.map((item: ingredient) => ({...item, inFilter: true}));
                }
                catch(error) {
                    // TODO popup error
                    console.error(error);
            ...
            const fetchAllIngredients = async () => {
                try {
                    ...
                    return response.map((item: ingredient) => ({...item, inFilter: false}));
                }
                catch(error) {
                    // TODO popup error
                    console.error(error);
            ...
            const cocktail_ingredients = await fetchCocktailIngredients();
            const all_ingredients = await fetchAllIngredients();

            setIngredientsState(all_ingredients.map((item: any) => cocktail_ingredients.find((item2: any) => item2.id === item.id) || item));
        };
    ...
    const onSave = useCallback(async () => {
        try {
            ...
            const response = await axios.post("/api/cocktails/update", formData, {
              headers: {
                "Content-Type": "multipart/form-data",
              },
            });
            ...
            if (response.status == 200) {
                router.push(`/view/${cocktailState?.id}`);
            }
        } catch (error) {
